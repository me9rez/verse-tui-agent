 ...
 *
 *   pnpm sessions   （= vitest run test/session.test.ts）
 *
...
import { createTranscriptStore } from '../src/transcript/index.ts'
import {
  SESSION_SCHEMA_V,
  asStoredSession,
  createTurnRecorder,
...
// ── 断言小工具：软断言 = 旧 check()「失败不阻断、最后统一算账」语义 ─────────
const check = (name: string, ok: boolean, detail: string): void => {
  expect.soft(ok, `${name} — ${detail}`).toBe(true)
}

// ── 每组断言写成一个函数，收集阶段注册成 vitest test ────────────────────────
...
// ── 注册 + 清理 ───────────────────────────────────────────────────────────
for (const [title, fn] of groups) test(title, fn)
